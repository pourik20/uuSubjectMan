...
const { Profile, AppClientTokenService, UuAppWorkspace, UuAppWorkspaceError, WorkspaceAuthorizationService, UuSubAppInstance } = require("uu_appg01_server").Workspace;
...
class SubjectManMainAbl {
  ...
  async init(uri, dtoIn, session) {
    ...
    if (dtoIn.uuBtLocationUri) {
      ...
      const createAwscDtoIn = {
        name: "UunSubjectMan",
        typeCode: "uun-subjectMan-maing01",
        location: location,
  ...
  async load(uri, session, uuAppErrorMap = {}) {
    ...
    // HDS 1
    dtoOut = await UuAppWorkspace.load(uri, session, uuAppErrorMap);
