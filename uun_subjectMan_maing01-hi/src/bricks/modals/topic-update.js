...
const TopicUpdate = createVisualComponent({
  //@@viewOn:statics
  uu5Tag: Config.TAG + "TopicUpdate",
  //@@viewOff:statics
  ...
  render(props) {
    ...
    useEffect(() => {
      ...
      setContent(props.topic.content.map((x) => x.name));
    }, []);
    ...
    const onSubmit = async () => {
      if (isValid()) {
        ...
        try {
          await topicContext.handlerMap.update(output);
